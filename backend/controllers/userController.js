...
export const sendCode = async (req, res) => {
  try {
    ...
    // ✅ Save to the fields that actually exist in your schema
    user.resetCode = resetCode;
    user.resetCodeExpire = Date.now() + 10 * 60 * 1000; // 10 minutes from now
    await user.save();
...
export const resetPassword = async (req, res) => {
  try {
    ...
    console.log("Reset request:", { email, codeStr });
    console.log("Stored resetCode:", user.resetCode);
    console.log("Expires at:", user.resetCodeExpire, "Now:", Date.now());

    // ✅ Check code value
    if (!user.resetCode || user.resetCode !== codeStr) {
    ...
    // This will be hashed by your pre-save hook in User model
